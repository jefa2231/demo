<section class="max-w-md mx-auto rounded-2xl border border-white/10 bg-slate-900 p-4 space-y-3">
  ...
  <form action="<?= e(local_url('/admin/login')) ?>" method="post" class="space-y-3">
    <div class="space-y-1">
      ...
      <input id="username" name="username" type="text" required autocomplete="username" class="w-full rounded-xl bg-slate-950 border border-white/10 px-3 py-3 text-sm text-slate-100 outline-none focus:border-emerald-500/60" />
    </div>
    <div class="space-y-1">
      <label class="text-xs text-slate-300" for="password">Password</label>
      <input id="password" name="password" type="password" required autocomplete="current-password" class="w-full rounded-xl bg-slate-950 border border-white/10 px-3 py-3 text-sm text-slate-100 outline-none focus:border-emerald-500/60" />
    </div>
